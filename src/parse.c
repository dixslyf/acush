...
parse_command_line(struct sh_parse_context *ctx, struct sh_ast **ast_out);

enum sh_parse_result
parse_job(struct sh_parse_context *ctx, struct sh_ast **ast_out);

enum sh_parse_result
parse_command(struct sh_parse_context *ctx, struct sh_ast **ast_out);

enum sh_parse_result
parse_simple_command(struct sh_parse_context *ctx, struct sh_ast **ast_out);
...
parse_command_line(struct sh_parse_context *ctx, struct sh_ast **ast_out) {
    ...
    {
        enum sh_job_type job_type = ctx->tokens[ctx->token_idx].type
                                            == SH_TOKEN_AMP
                                        ? SH_JOB_BG
                                        : SH_JOB_FG;
        ctx->token_idx++;

        jobs[job_count] = (struct sh_job) {
            .ast_node = job_node,
            .job_type = job_type,
        };
        job_count++;
        ...
        if (job_count == jobs_capacity) {
            jobs_capacity *= 2;
            struct sh_job *jobs_tmp = realloc(
                jobs,
                sizeof(struct sh_job) * jobs_capacity
            );
            if (jobs_tmp == NULL) {
                free(jobs);
                return SH_PARSE_MEMORY_ERROR;
...
parse_job(struct sh_parse_context *ctx, struct sh_ast **ast_out) {
           ...
           && ctx->tokens[ctx->token_idx].type == SH_TOKEN_PIPE)
    {
        ...
        if (cmd_count == cmds_capacity) {
            cmds_capacity *= 2;
            struct sh_ast **cmds_tmp = realloc(
                cmds,
                sizeof(struct sh_ast *) * cmds_capacity
            );
            if (cmds_tmp == NULL) {
                free(cmds);
                return SH_PARSE_MEMORY_ERROR;
...
parse_command(struct sh_parse_context *ctx, struct sh_ast **ast_out) {
    ...
    enum sh_parse_result parse_simple_cmd_result = parse_simple_command(
        ctx,
        &simple_command_node
    );
    ...
    struct sh_ast *node = init_ast_node(
        ...
        (union sh_ast_data) {
            .command =
                {
                    .simple_command = simple_command_node,
    ...
    case SH_AST_COMMAND_LINE:
        ...
        if (ast->data.command_line.type == SH_COMMAND_REPEAT) {
            ...
            for (size_t idx; idx < ast->data.command_line.job_count; idx++) {
                printf(
                    "    %s ",
                    ast->data.command_line.jobs[idx].job_type == SH_JOB_FG
                        ? "FOREGROUND"
                        : "BACKGROUND"
                );
